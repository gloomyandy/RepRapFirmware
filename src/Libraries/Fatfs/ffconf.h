...
#define FF_USE_LFN		2
#define FF_MAX_LFN		120					// this should be >= MaxFilenameLength in Configuration.h. Higher values use more stack space, so don't make it too large.
...
// FF_DISKIO_ALIGN defines the alignment required by the disk interface hardware. Set to 1 (no alignment), 2 (16-bit alignment) or 4 (32-bit alignment).
// For example, the SAM3XA build of the HSMCI driver in Atmel Software Framework requires 32-bit alignment because of the way it uses DMA.
#define FF_DISKIO_ALIGN		(4)
